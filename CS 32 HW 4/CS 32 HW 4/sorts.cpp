...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    for(int i = 1; i < s.size(); i++)
    {
        Sensor nextItem = s[i];
        int j = i;
        while((j > 0) && (comp(s[j-1], nextItem) == false))
        {
            s[j] = s[j-1];
        }
        s[j] = nextItem;
...
int main()
{
    ...
    // destruction time.
    {
        ...
        //        order.
        for(int i = 0; i < pointerSensors.size(); i++)
        {
            sensors[i] = *pointerSensors[i];
        }
